...
<?php
    ...
    <div class="train">
        ...
        <div>
        <h3>Train No/Name</h3>
        <p><?php echo $train_no.'/'.$train_name; ?></p>
        </div>
...
<script>
      const train_no = "<?php echo $train_no; ?>";
     const train_name = "<?php echo $train_name; ?>";
     ...
     const train_class = "<?php echo $class; ?>";
     const pnr = "<?php echo $pnr; ?>";
     const booking_date = "<?php echo $booking_date; ?>";
     const journey_date = "<?php echo $journey_date; ?>";
     let index = 0;

     const passenger = <?php echo json_encode($passenger); ?>;
     
        document.addEventListener("DOMContentLoaded",function(){trainticket();});

            function trainticket()
            {       
                const utterance = new SpeechSynthesisUtterance(
                `The Train Number is ${train_no}, Train Name is ${train_name}, your start station is ${source}, 
                your destination station is ${destination}, starting at ${start_time} and arriving at ${end_time}, 
                the class is ${train_class}, your PNR number is ${pnr}, the Booking date is ${booking_date},
                 your journey date is ${journey_date}`);
                ...
                function readpass()
                {
                    ...
                    {
                    const currentPassenger = passenger[index]; 
                    const utterance = new SpeechSynthesisUtterance(
                    ` Passenger ${index+1}, Name is: ${currentPassenger.name},compartment is: ${currentPassenger.compartment},
                    Seat number is: ${currentPassenger.seat}, and booking status is: ${currentPassenger.booking_status}`);
                    utterance.lang = "en-IN";
                    speechSynthesis.speak(utterance);
                    utterance.onend = () => 
                    {
                    ...
                    {
                        const promptUtterance = new SpeechSynthesisUtterance("can I Repeat Once more");
